...
- ☐ **Port MIDI de retour `OpenLamp Feedback`** (moteur → DAW) : s'abonner au WebSocket d'état WLED (`ws://<lampe>/ws`) et **ré-émettre l'état des lampes en MIDI** (CC = brillance/hue/sat, note = on/off, CC = index d'effet) → débloque le **feedback value-level en Tier 1** (n'importe quel DAW, sans M4L : l'utilisateur MIDI-mappe les CC entrants). Le rendu riche (couleur/nom d'effet) reste Tier 2 (M4L). Consommé côté Live — voir [openlamp/live TASKS.md](https://github.com/openlamp/live/blob/main/TASKS.md).
